...
dist: $(DIST_LANGUAGES) release $(CATALOGS)
	@-delete RAM:$(OUTFILE) RAM:$(OUTFILE).lha ALL FORCE QUIET
	@mkdir -p RAM:$(OUTFILE)/gfx >NIL:
	@copy $(OUTFILE) RAM:$(OUTFILE)/$(OUTFILE) >NIL:
	@copy doc/$(OUTFILE).readme RAM:$(OUTFILE)/$(OUTFILE).readme >NIL:
	@copy SYS:Prefs/Presets/Deficons/def_drawer.info RAM:$(OUTFILE).info
	@copy $(OUTFILE).info RAM:$(OUTFILE)/$(OUTFILE).info >NIL:
	@$(foreach LANG, $(CATALOGS:locale/%.catalog=%), mkdir -p RAM:$(OUTFILE)/catalogs/$(LANG))
	@$(foreach LANG, $(CATALOGS:locale/%.catalog=%), copy locale/$(LANG).catalog RAM:$(OUTFILE)/catalogs/$(LANG)/$(OUTFILE).catalog)
	@copy gfx RAM:$(OUTFILE)/gfx/ ALL >NIL:
